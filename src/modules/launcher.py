...
def set_progress(page: ft.Page, progress: int, progress_bar: ft.ProgressBar, progress_text: ft.Text):
    if current_max != 0:
        progress_bar.value = progress / current_max
        progress_bar.update()
        update_status_safe(page, progress_text, f"{progress}/{current_max}")
...
def install_version(page: ft.Page, version_id: str, buttons_to_disable: list, progress_window: ft.AlertDialog,
                    progress_bar: ft.ProgressBar, status_text: ft.Text, progress_text: ft.Text, refresh):
    try:
        set_controls_enabled_safe(page, buttons_to_disable, False)
        ...
        callback={
            ...
            "setProgress": lambda progress: set_progress(page, progress, progress_bar, progress_text),
            "setMax": set_max
            }
...
def get_versions() -> dict:
    """Returns a dict of all Minecraft versions (release, snapshot)"""
